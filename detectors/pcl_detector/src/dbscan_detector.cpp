...
std::vector<pcl::PointCloud<pcl::PointXYZ>> DBSCANDetector::get_clusters(const pcl::PointCloud<pcl::PointXYZ>& points)
{
    // Create a KD-Tree
    pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
    tree->setInputCloud(points.makeShared());
...
pcl::PointCloud<pcl::PointXYZ> DBSCANDetector::get_centroids(const std::vector<pcl::PointCloud<pcl::PointXYZ>>& clusters){
    pcl::PointCloud<pcl::PointXYZ> centroids;
    for (const auto& cluster : clusters) {
        pcl::PointXYZ centroid;
        pcl::computeCentroid(cluster, centroid);
