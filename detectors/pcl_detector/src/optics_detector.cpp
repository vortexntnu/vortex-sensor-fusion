...
pcl::PointCloud<pcl::PointXYZ> OPTICSDetector::get_detections(const pcl::PointCloud<pcl::PointXYZ>& points) {
    ...
    std::vector<float> reachability_distance(points.size(), std::numeric_limits<float>::infinity());
...
void OPTICSDetector::expand_cluster(const pcl::PointCloud<pcl::PointXYZ>& points,
    ...
    std::vector<float> core_distances(neighbors.size());
    for (size_t i = 0; i < neighbors.size(); ++i) {
        ...
        if (!visited[neighbor_idx]) {
            ...
            if (neighbor_neighbors.size() >= min_pts_) {
                std::vector<float> neighbor_core_distances(neighbor_neighbors.size());
                ...
                float core_distance = neighbor_core_distances[min_pts_ - 1];
                core_distances[i] = core_distance;
            } else {
                core_distances[i] = std::numeric_limits<float>::infinity();
            }
        } else {
